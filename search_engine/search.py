...
class songRes:
    def GET(self):
        ...
        if order==u"commentcount":
            order=u"commentsNum";
        if order==u"sharecount":
            order=u"maxlikedcount";
...
class playlistRes:
    def GET(self):
        ...
        key=user_data.keyword
        order=user_data.order
        if order==u"commentsNum":
            order=u"commentcount";
        if order==u"maxlikedcount":
            order=u"sharecount";   
...
class commentRes:
    def GET(self):
        ...
        key=user_data.keyword
        order=user_data.order   
        ResultNum, ResultData = search_playlist(user_data.keyword,"","")
