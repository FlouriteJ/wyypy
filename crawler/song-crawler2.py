...
def getSong2(idSong = "246316"):
	global fileSong,lock,threads,hashSongvisited,success,fail
	urlSong = 'http://music.163.com/song?id='
	try:
		r = requests.get(urlSong + idSong,headers = headers,timeout = 2)
		text = r.text
		...
		t = ','.join([idSong,title,author,album,image,idArtist,idAlbum])
		if lock.acquire():
...
#Initialization
if os.path.exists('song_visit2.db'):
	hashSongvisited = pickle.load(open('song_visit2.db','rb'))
else:
	hashSongvisited = {}
...
f = open('song.db','r')
fileSong = open('song_details2.db','ab')
maxThreads = 500
threads = 0
lock = threading.Lock()
count = 1
	...
	if hashSongvisited.get(id,False)==False:
		...
		if count%100==0:
			...
			try:
				...
				print("threads= ",threads,'\t',len(hashSongvisited),'\t','time= %.2f'%(time.time()-last),'\t%.2f%%'%(beta*100))
				success = 0
				fail = 0
			except:
				pass
			last = time.time()
		if count>=2000:
			pickle.dump(hashSongvisited,open('song_visit2.db','wb'))
			print('-'*10+'pickled'+'-'*10)
			count-=2000
while True:
	time.sleep(0.5)
